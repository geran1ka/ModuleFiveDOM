...
const titleRow = createElement('th', {
...
const createRow = ({id, name, category, units, count, price, total, images}) => {
  const tableRow = createElement('tr', {
    className: 'table__row',
  }, {
    parent: tBody,
  })
  for (const key in {id, name, category, units, count, price, total, images}) {
    let text = 0;
    if (key === 'total') {
      text = '$' + (onj.count * onj.price);
    } else if (key === 'images') {
    ...
    createElement('td', {
      className: `table__cell table__cell-${key}`,
      textContent: `${text}`
    }, {
      parent: tableRow
    })
